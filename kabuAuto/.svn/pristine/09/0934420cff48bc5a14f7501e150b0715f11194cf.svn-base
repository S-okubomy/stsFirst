...
<title>GAの計算値をボリンジャー・バンドにセット</title>
...
<h5>GAの学習結果</h5>
<font size="1">
	calListDto.bolCalLow = (ac1[i][2] * calListDto.transAve20 - ac1[i][0] * calListDto.sigma20) * ac1[i][3];<br>
	calListDto.bolCalUp = (ac1[i][2] * calListDto.transAve20 + ac1[i][0] * calListDto.sigma20) * ac1[i][3];
</font>

	<table border="1" >
		<tr>
			<th class="textRight">学習世代数</th>
			<th>個体No</th>
			<th>最終適応度</th>
			<th>係数0 A0</th>
			<th>係数1 A1</th>
		...
		<tr>
			<td><fmt:formatNumber value="${gSedai}" pattern="0" /></td>
			<td><fmt:formatNumber value="${pcsNo}" pattern="0" /></td>
			<td><fmt:formatNumber value="${fitLast}" pattern="0.0" /></td>
			<td><fmt:formatNumber value="${A0}" pattern="0.0" /></td>
			<td><fmt:formatNumber value="${A1}" pattern="0.0" /></td>
			<td><fmt:formatNumber value="${A2}" pattern="0.0" /></td>
			<td><fmt:formatNumber value="${A3}" pattern="0.0" /></td>
		</tr>
	</table>

<br>
<h4>バンド下部 該当リスト（買いかも）</h4>
	<table border="1" >
		<tr>
